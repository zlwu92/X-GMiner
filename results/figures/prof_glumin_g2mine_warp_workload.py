...
data = pd.read_csv('../work_depth_per_warp_glumin_lut.csv', header=None)

for index, row in data.iterrows():
    # 提取标签和数值
    group_name = row[0]  # 第一列（如 testgr2）
    subgroup_name = row[1]  # 第二列（如 P2_LUT）
    values = row[2:].dropna().astype(int).tolist()  # 从第三列开始提取数值
...
def prof_glumin_g2miner_workload():
    # 获取所有唯一的 Pattern 值
    ...
    index = np.arange(len(values))  # x 轴索引
    ax.set_ylim(0, 5)
    ax.yaxis.set_ticks([0, 1, 2, 3, 4, 5])
    ax.bar(index, values, bar_width, label='', color=mycolors[0])
    
    ax.set_xticks(index)
    ax.set_xlabel(f'Warp IDs', fontsize=14)
    ax.set_ylabel('Loop Depth', fontsize=12)
    ...
    plt.savefig(f'prof_glumin_g2miner_workload_{current_date}.pdf', dpi=300, bbox_inches='tight')


if __name__ == '__main__':
    prof_glumin_g2miner_workload()