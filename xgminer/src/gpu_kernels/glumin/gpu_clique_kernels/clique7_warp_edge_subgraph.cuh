...
__global__ __inline__ void __launch_bounds__(BLK_SZ, 8)
clique7_warp_edge_subgraph(eidType ne, GraphGPU g, vidType *vlists, vidType max_deg, AccType *total, BinaryEncode<> sub_graph) {
  ...
  bitmapType v1v2[32], v1v2v3[32];
  bitmapType v1v2_reg, v1v2v3_reg;
          ...
          if (sub_graph_shared[warp_lane][v1] & (1 << v2) ){
            ...
            for (vidType v3 = 0; v3 < list_size[warp_lane]; v3++) {
              if (v1v2_reg & (1 << (v3 % BITMAP_WIDTH))) {
                ...
                v1v2v3_reg = v1v2_reg & sub_graph_shared[warp_lane][v3];
                for (vidType v4 = 0;v4 < list_size[warp_lane]; v4++) {
                    if (v1v2v3_reg & (1 << (v4 % BITMAP_WIDTH))) {
                        counter += __popc(v1v2v3_reg & sub_graph_shared[warp_lane][v4]);
                    }
                }
              }
            }
          ...
          if (sub_graph.get(warpMapHead, v1, v2)){
            ...
            for (vidType v3 = 0; v3 < list_size[warp_lane]; v3++) {
              ...
              if (v1v2[v3 / BITMAP_WIDTH] & (1 << (v3 % BITMAP_WIDTH))) {
                // Use register result of v1 & v2, compute (v1 & v2) & v3
                sub_graph.intersect_thread(warpMapHead, countElement, v1v2, v3, v1v2v3);
                for (vidType v4 = 0; v4 < list_size[warp_lane]; v4++) {
                  if (v1v2v3[v4 / BITMAP_WIDTH] & (1 << (v4 % BITMAP_WIDTH))) {
                    counter += sub_graph.intersect_num_thread_pre(warpMapHead, countElement, v1v2v3, v4);
                  }
                }
              }
            }
