#define __N_LISTS2 4
#define __N_BITMAPS2 2

__global__ void __launch_bounds__(BLOCK_SIZE, 8)
GM_LUT_warp_deep(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
  ...
  for (eidType e01_idx = begin + warp_id; e01_idx < ne; e01_idx += num_warps){
    ...
    for (vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx++) {
      ...
      for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
        ...
        count += __difference_num(__get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/false, /*upper_bound=*/-1), __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/false, /*upper_bound=*/-1), /*upper_bound=*/-1);
      }
    }
  }
  // END OF CODEGEN
...
__global__ void __launch_bounds__(BLOCK_SIZE, 8)
GM_LUT_block_deep(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
...
__global__ void __launch_bounds__(BLOCK_SIZE, 8)
GM_LUT_block_large_deep(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
