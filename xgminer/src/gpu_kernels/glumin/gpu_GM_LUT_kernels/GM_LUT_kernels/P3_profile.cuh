...
P3_GM_LUT_block_workload_test(vidType begin, vidType end, 
                            vidType *vid_list,
                            GraphGPU g, 
                            vidType *vlists,
                            bitmapType* bitmaps,
        ...
        for (vidType v1_idx = warp_lane; v1_idx < candidate_v1.size(); v1_idx += WARPS_PER_BLOCK) {
            ...
            __build_index_from_vmap_test(g, meta, 
                    __get_vmap_from_lut_vid_limit_test(g, meta, /*idx_id=*/v1_idx, 
                                    /*connected=*/true, /*upper_bound=*/v1, workload), 
                    /*slot_id=*/2, workload);
            ...
            for(vidType v2_idx_idx = thread_lane; v2_idx_idx < candidate_v2_idx.size(); v2_idx_idx += WARP_SIZE){
                ...
                count += __difference_num(
                            ...
                            /*upper_bound=*/-1);
                auto bitmap1 = __get_vmap_from_lut(g, meta, /*idx_id=*/v1_idx, /*connected=*/false, /*upper_bound=*/-1).bitmap_;
                auto bitmap2 = __get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/false, /*upper_bound=*/-1).bitmap_;
                auto size = bitmap1.size() < bitmap2.size() ? bitmap1.size() : bitmap2.size();
                workload[threadIdx.x + block_id * blockDim.x] += (size / BITMAP_WIDTH + 1);
            }
        }
        __syncthreads();
    }
