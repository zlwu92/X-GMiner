...
P6_GM_LUT_block_workload_test(vidType begin, vidType end, 
                            vidType *vid_list,
                            GraphGPU g, 
                            vidType *vlists,
                            bitmapType* bitmaps,
    ...
    for(vidType v0_idx = begin + block_id; v0_idx < candidate_v0.size(); v0_idx += num_blocks){
        ...
        for (vidType v1_idx = warp_lane; v1_idx < candidate_v1.size(); v1_idx += WARPS_PER_BLOCK) {
            ...
            __build_index_from_vmap_test(g, meta, 
                    __get_vmap_from_lut_vid_limit(g, meta, /*idx_id=*/v1_idx, /*connected=*/true, /*upper_bound=*/v1), 
                    /*slot_id=*/3, workload);
            ...
            for(vidType v2_idx_idx = 0; v2_idx_idx < candidate_v2_idx.size(); v2_idx_idx ++){
                ...
                __difference(meta, 
                    ...
                    /*upper_bound=*/v1_idx, /*output_slot=*/0);
                workload[threadIdx.x + block_id * blockDim.x] += (v1_idx / BITMAP_WIDTH + 1);
                __build_index_from_vmap_test(g, meta,
                             __get_vmap_from_heap(g, meta, /*bitmap_id=*/0, /*slot_id=*/-1), /*slot_id=*/4, workload);
                ...
                for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
                    ...
                    count += __intersect_num(
                                __get_vmap_from_heap(g, meta, /*bitmap_id=*/0, /*slot_id=*/-1), 
                                __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/true, /*upper_bound=*/-1), 
                                /*upper_bound=*/v3_idx);
                    workload[threadIdx.x + block_id * blockDim.x] += (v3_idx / BITMAP_WIDTH + 1);
                }
            }
        }
        __syncthreads();    
