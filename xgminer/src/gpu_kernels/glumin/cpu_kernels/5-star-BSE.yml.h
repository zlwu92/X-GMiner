...
void BS_vertex_5star(Graph_V2 &g, StorageMeta &meta, uint64_t &total){
  uint64_t counter = 0;
  ...
  for(eidType eid = 0; eid < g.E(); eid += 1){
    ...
    for(vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx ++){
      ...
      __difference(meta, __get_vlist_from_heap(g, meta, /*slot_id=*/0), __get_vlist_from_graph(g, meta, /*vid=*/v2), /*upper_bound=*/v2, /*output_slot=*/1);
      ...
      for(vidType v3_idx = 0; v3_idx < candidate_v3.size(); v3_idx ++){
        ...
        counter += __difference_num(__get_vlist_from_heap(g, meta, /*slot_id=*/1), __get_vlist_from_graph(g, meta, /*vid=*/v3), /*upper_bound=*/v3);
      }
    }
  }
  // END OF CODEGEN
