...
void BS_vertex_5anti_tailed_diamond(Graph_V2 &g, StorageMeta &meta, uint64_t &total)
{
  ...
  for(eidType eid = 0; eid < g.E(); eid += 1){
    ...
    for(vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx ++){
      ...
      for(vidType v3_idx = 0; v3_idx < candidate_v3.size(); v3_idx ++){
        ...
        counter += __difference_num(__get_vlist_from_heap(g, meta, /*slot_id=*/3), __get_vlist_from_graph(g, meta, /*vid=*/v3), /*upper_bound=*/-1);
      }
    }
  }
  // END OF CODEGEN
