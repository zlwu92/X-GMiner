#include "graph_v2.h"
#include "../include/glumin/scan.h"
#include "../include/glumin/timer.h"

Graph_V2::Graph_V2(std::string prefix, bool use_dag, bool directed, 
    ...
    // 为每个顶点分配唯一的“方向性”，使得所有边从低度数顶点指向高度数顶点
    //（或在度数相等时，从小编号顶点指向大编号顶点）。
    if (use_dag) {
...
void Graph_V2::build_reverse_graph() {
...
VertexSet Graph_V2::out_neigh(vidType vid, vidType offset) const {
  assert(vid >= 0);
  assert(vid < n_vertices);
...
VertexSet Graph_V2::in_neigh(vidType vid) const {
  assert(vid >= 0);
  assert(vid < n_vertices);
...
void Graph_V2::allocateFrom(vidType nv, eidType ne) {
  n_vertices = nv;
  n_edges    = ne;
...
vidType Graph_V2::compute_max_degree() {
...
void Graph_V2::print_graph() const {
...
bool Graph_V2::is_connected(vidType v, vidType u) const {
...
bool Graph_V2::is_connected(std::vector<vidType> sg) const {
  return false;
}

bool Graph_V2::binary_search(vidType key, eidType begin, eidType end) const {
  auto l = begin;
  auto r = end-1;
  while (r >= l) { 
    auto mid = l + (r - l) / 2;
...
vidType Graph_V2::intersect_num(vidType v, vidType u, vlabel_t label) {
  vidType num = 0;
...
vidType Graph_V2::intersect_num(VertexSet& vs, vidType u, vlabel_t label) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType u_size = get_degree(u);
  vidType* u_ptr = &edges[vertices[u]];
  while (idx_l < vs.getsize() && idx_r < u_size) {
    vidType a = vs[idx_l];
    vidType b = u_ptr[idx_r];
    if (a <= b) idx_l++;
    if (b <= a) idx_r++;
...
vidType Graph_V2::intersect_set(vidType v, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType v_size = get_degree(v);
  vidType u_size = get_degree(u);
...
vidType Graph_V2::intersect_set(VertexSet& vs, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType u_size = get_degree(u);
  vidType* u_ptr = &edges[vertices[u]];
...
vidType Graph_V2::difference_num_edgeinduced(vidType v, vidType u, vlabel_t label) {
  vidType num = 0;
  vidType* v_ptr = &edges[vertices[v]];
...
vidType Graph_V2::difference_num_edgeinduced(VertexSet& vs, vidType u, vlabel_t label) {
  vidType num = 0;
  for (auto w : vs)
    if (w != u && vlabels[w] == label) num++;
  return num;
...
vidType Graph_V2::difference_set_edgeinduced(vidType v, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  vidType* v_ptr = &edges[vertices[v]];
  for (vidType i = 0; i < get_degree(v); i ++) {
    auto w = v_ptr[i];
...
vidType Graph_V2::difference_set_edgeinduced(VertexSet& vs, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  for (auto w : vs) {
    if (w != u && vlabels[w] == label) {
      result.add(w);
...
vidType Graph_V2::difference_num(vidType v, vidType u, vlabel_t label) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType v_size = get_degree(v);
  vidType u_size = get_degree(u);
...
vidType Graph_V2::difference_num(VertexSet& vs, vidType u, vlabel_t label) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType u_size = get_degree(u);
  vidType* u_ptr = &edges[vertices[u]];
...
vidType Graph_V2::difference_set(vidType v, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType v_size = get_degree(v);
  vidType u_size = get_degree(u);
...
vidType Graph_V2::difference_set(VertexSet& vs, vidType u, vlabel_t label, VertexSet& result) {
  vidType num = 0;
  vidType idx_l = 0, idx_r = 0;
  vidType u_size = get_degree(u);
  vidType* u_ptr = &edges[vertices[u]];
...
void Graph_V2::BuildReverseIndex() {
...
void Graph_V2::computeLabelsFrequency() {
...
int Graph_V2::get_frequent_labels(int threshold) {
  int num = 0;
  for (size_t i = 0; i < labels_frequency_.size(); ++i)
...
bool Graph_V2::is_freq_vertex(vidType v, int threshold) {
  assert(v >= 0 && v < size());
...
void Graph_V2::print_meta_data() const {
