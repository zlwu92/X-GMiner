#pragma once
#include "graph_v2.h"
#include "glumin/operations.cuh"
#include "cuda_utils/cuda_utils.cuh"
#include "cuda_utils/gpu_utils.h"
...
public:
  ...
  GraphGPU(Graph_V2 &g) : device_id(0), n_gpu(1) { init(g); }
  GraphGPU(Graph_V2 &g, int n, int m) : device_id(n), n_gpu(m) { init(g); }
  ...
  void init(Graph_V2 &g, int n, int m) {
    device_id = n;
    n_gpu = m;
    init(g);
  }
  void init(Graph_V2 &hg) {
  ...
  void copy_edgelist_to_device(size_t nnz, Graph_V2 &hg, bool sym_break = false) {
    copy_edgelist_to_device(0, nnz, hg, sym_break);
  }
  void copy_edgelist_to_device(size_t begin, size_t end, Graph_V2 &hg, bool sym_break = false) {
  ...
  void init_edgelist_um(Graph_V2 &g, bool sym_break = false) {
    Timer t;
    t.Start();
    size_t i = 0;
