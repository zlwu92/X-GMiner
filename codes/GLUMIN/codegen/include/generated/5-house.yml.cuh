...
generated_kernel(vidType nv, 
  ...
  for(vidType v0_idx = warp_id; v0_idx < candidate_v0.size(); v0_idx += num_warps){
    ...
    for (vidType v1_idx = 0; v1_idx < candidate_v1.size(); v1_idx += 1) {
      ...
      __build_index_from_vmap(g, meta, __get_vmap_from_lut_vid_limit(g, meta, /*idx_id=*/v1_idx, /*connected=*/true, /*upper_bound=*/v1), /*slot_id=*/5);
      __difference(meta, __get_vlist_from_graph(g, meta, /*vid=*/v1), __get_vlist_from_graph(g, meta, /*vid=*/v0), /*upper_bound=*/-1, /*output_slot=*/1);
      auto candidate_v2_idx = __get_vmap_from_heap(g, meta, /*bitmap_id=*/-1, /*slot_id=*/5);
      for(vidType v2_idx_idx = 0; v2_idx_idx < candidate_v2_idx.size(); v2_idx_idx ++){
        ...
        __difference(meta, __get_vlist_from_graph(g, meta, /*vid=*/v2), __get_vlist_from_graph(g, meta, /*vid=*/v1), /*upper_bound=*/-1, /*output_slot=*/3);
        __difference(meta, __get_vlist_from_heap(g, meta, /*slot_id=*/3), __get_vlist_from_graph(g, meta, /*vid=*/v0), /*upper_bound=*/-1, /*output_slot=*/4);
        ...
        for(vidType v3_idx = 0; v3_idx < candidate_v3.size(); v3_idx ++){
          ...
          count += __intersect_num(__get_vlist_from_heap(g, meta, /*slot_id=*/4), __get_vlist_from_graph(g, meta, /*vid=*/v3), /*upper_bound=*/-1);
        }
      }
    }
  }
