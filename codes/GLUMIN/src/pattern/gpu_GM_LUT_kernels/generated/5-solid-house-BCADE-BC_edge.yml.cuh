...
GM_LUT_warp(eidType begin, eidType ne, 
  ...
  for (eidType e01_idx = begin + warp_id; e01_idx < ne; e01_idx += num_warps){
    ...
    auto v1_size = g.getOutDegree(v1);
    if (v0_size < v1_size) continue;
    ...
    for (vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx++) {
      ...
      for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
        ...
        count += __intersect_num(__get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/false, /*upper_bound=*/-1), __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/true, /*upper_bound=*/-1), /*upper_bound=*/-1);
      }
    }
  }
  // END OF CODEGEN
