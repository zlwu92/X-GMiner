...
GM_LUT_warp(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
  ...
  for (eidType e01_idx = begin + warp_id; e01_idx < ne; e01_idx += num_warps){
    ...
    for(vidType v2_idx = 0; v2_idx < candidate_v2.size(); v2_idx++){
      __build_index_from_vmap(g, meta, __get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/true, /*upper_bound=*/v2_idx), /*slot_id=*/2);
      ...
      for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
        ...
        count += __difference_num(__get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/true, /*upper_bound=*/-1), __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/false, /*upper_bound=*/-1), /*upper_bound=*/-1);
      }
    }
  }
  // END OF CODEGEN
...
#ifdef PROF
  if (thread_lane == 0) {
    size_t lutmem = get_maxdeg * ((get_maxdeg + 32 - 1) / 32) * sizeof(BITMAP_WIDTH);
    atomicAdd(&counter[1], lutmem);
  }
#endif
}

__global__ void __launch_bounds__(BLOCK_SIZE, 8)
GM_LUT_block(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
  ...
  for (eidType e01_idx = begin + block_id; e01_idx < ne; e01_idx += num_blocks){
    ...
    auto candidate_v2 = __get_vlist_from_heap(g, meta, /*slot_id=*/0);
#ifdef PROF
    if (candidate_v2.size() > get_maxdeg) get_maxdeg = candidate_v2.size();
#endif   
  ...
  if (threadIdx.x == 0) {
    size_t lutmem = get_maxdeg * ((get_maxdeg + 32 - 1) / 32) * sizeof(BITMAP_WIDTH);
    atomicAdd(&counter[2], lutmem);
  }
#endif
}

__global__ void __launch_bounds__(BLOCK_SIZE, 8)
GM_LUT_block_large(eidType begin, eidType ne, 
                  GraphGPU g, 
                  vidType *vlists,
                  bitmapType* bitmaps,
                  vidType max_deg,
  ...
  for (eidType e01_idx = begin + block_id; e01_idx < ne; e01_idx += num_blocks){
    ...
    for (vidType v2_idx = warp_lane; v2_idx < candidate_v2.size(); v2_idx += WARPS_PER_BLOCK) {
      __build_index_from_vmap(g, meta, __get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/true, /*upper_bound=*/v2_idx), /*slot_id=*/2);
      // __build_index_from_vmap(g, meta, __get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/false, /*upper_bound=*/-1), /*slot_id=*/3);
      auto candidate_v3_idx = __get_vmap_from_heap(g, meta, /*bitmap_id=*/-1, /*slot_id=*/2);
      for(vidType v3_idx_idx = thread_lane; v3_idx_idx < candidate_v3_idx.size(); v3_idx_idx += WARP_SIZE){
        auto v3_idx = candidate_v3_idx[v3_idx_idx];
        count += __difference_num(__get_vmap_from_lut(g, meta, /*idx_id=*/v2_idx, /*connected=*/true, /*upper_bound=*/-1), __get_vmap_from_lut(g, meta, /*idx_id=*/v3_idx, /*connected=*/false, /*upper_bound=*/-1), /*upper_bound=*/-1);
